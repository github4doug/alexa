  ...
  if (!req.session('hasWeapon')) {
    ...
    var dragon = req.session('dragon');
        if (dragon.type.toLowerCase() != req.slot('DragonType').toLowerCase()) {
          ...
          res.say("I don't see a " + req.slot('DragonType') + ' dragon anywhere! \
Just a ' + dragon.type + ' one.');
          res.shouldEndSession(false);
        }
        else {
          ...
          if (dragon.hp <= 0) {
            res.say('You have slain the mighty ' + dragon.type + ' dragon! \
Congratulations!');
            res.shouldEndSession(true);
          }
          else {
            res.say("You attack the " + dragon.type + " dragon with your lance! \
It has " + dragon.hp + ' HP left!');
            res.shouldEndSession(false);
          }
        }
      }
